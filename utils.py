...
# height_image, width_image = 512, 512
...
def create_input_files(
    image_folder="PubTabNet/pubtabnet",
    ...
    height_image: max image height
    width_image: max image width
    max_len_token_structure: don't sample captions_structure longer than this length
    max_len_token_cell: don't sample captions_structure longer than this length
    """
    ...
    for (index, image) in enumerate(imgs):
        ...
        captions_structure = []
        caption_cells = []
        path = os.path.join("{}/{}".format(image_folder, img["split"]), img["filename"])
        # print(
        #     "check_longgest_cell: {}".format(check_longest_cell(img["html"]["cells"]))
        # )
        if (
            len(img["html"]["structure"]["tokens"]) <= max_len_token_structure
            and check_longest_cell(img["html"]["cells"]) <= max_len_token_cell
        ):
            ...
            if (
                img["split"] == "train"
                and total_number_imgs_train <= max_number_imgs_train
            ):
    ...
    with open(
        os.path.join(output_folder, "WORDMAP_" + "STRUCTURE" + ".json"), "w"
    ) as j:
    ...
    for impaths, imcaps_structure, imcaps_cell, split in [
        (
            train_image_paths,
            train_image_captions_structure,
            train_image_captions_cells,
        ...
        with h5py.File(os.path.join(output_folder, split + "_IMAGES_.hdf5"), "a") as h:
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                # img = imreade(impaths[i])
                img = np.array(img)
                if len(img.shape) == 2:
                    img = img[:, :, np.newaxis]
                    img = np.concatenate([img, img, img], axis=2)
                # img = imresize(img, (width_image, height_image), interp="cubic")
                img = Image.fromarray(np.uint8(img))
                img = img.resize((width_image, height_image), resample=3)
                img = np.array(img)
                ...
                for j, c in enumerate(captions_structure):
                    enc_c = (
                        [word_map_structure["<start>"]]
                        + [
                            word_map_structure.get(word, word_map_structure["<unk>"])
                            for word in c
                        ]
                        + [word_map_structure["<end>"]]
                        + [word_map_structure["<pad>"]]
                        * (max_len_token_structure - len(c))
                    )
                    c_len = len(c) + 2
                ...
                for j, c in enumerate(captions_cell):
                    enc_c = (
                        ...
                        + [word_map_cell["<end>"]]
                        + [word_map_cell["<pad>"]] * (max_len_token_cell - len(c))
                    )
                    c_len = len(c) + 2
                ...
                padding_enc_caption_cell = [
                    [0 for y in range(max_len_token_cell + 2)]
                    for x in range(max_cells_per_images - len(each_enc_captions_cell))
                ]

                padding_len_caption_cell = [
                    0 for x in range(max_cells_per_images - len(each_enc_captions_cell))
                ]
            ...
            with open(
                os.path.join(output_folder, split + "_CAPTIONS_STRUCTURE" + ".json"),
                "w",
            ) as j:
                json.dump(enc_captions_structure, j)
            with open(
                os.path.join(output_folder, split + "_CAPLENS_STRUCTURE" + ".json"), "w"
            ) as j:
                json.dump(cap_structure_len, j)
            with open(
                os.path.join(output_folder, split + "_CAPTIONS_CELL" + ".json"), "w"
            ) as j:
                json.dump(enc_captions_cells, j)
            with open(
                os.path.join(output_folder, split + "_CAPLENS_CELL" + ".json"), "w"
            ) as j:
                json.dump(cap_cell_len, j)
            with open(
                os.path.join(
                    output_folder, split + "_NUMBER_CELLS_PER_IMAGE" + ".json"
                ),
                "w",
            ) as j:
...
def save_checkpoint(
    ...
    decoder_cell,
    encoder_optimizer,
    decoder_structure_optimizer,
    decoder_cell_optimizer,
    recent_ted_score,
    is_best,
):
    """
    ...
    recent_ted_score: validation TED score for this epoch
